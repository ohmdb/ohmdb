...
public class JoinInitializer implements Comparator<JoinSide> {

	public static final JoinInitializer INSTANCE = new JoinInitializer();

	private JoinInitializer() {
	}
		...
		if (withIds == 0) {
			...
			if (minJoin != null) {
				if (minFrom) {
					...
					sides[minJoin.from].futureIds = UTILS.futureIds(ids);
				} else {
					// System.out.println("MIN TO JOIN " + minJoin +
					// ", MIN SIZE: " + minSize + " :: " + minFrom);
					Numbers ids = minJoin.rel.tos();
					sides[minJoin.to].futureIds = UTILS.futureIds(ids);
				}
			} else {
				return false;
			}
	...
	private void initBridges(JoinSide[] sides) {
		for (int i = 0; i < sides.length; i++) {
			...
			side.bridge = new JoinBridge(fromRels, toRels);
		}
	}
